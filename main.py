...
import Ui_addEditCoffeeForm, Ui_main
...
class MainWindow(QMainWindow, Ui_main.Ui_MainWindow, DbBasicFunctions):
    def __init__(self):
        super().__init__()
        if isfile("./data/coffee.sqlite"):
...
class AddEditCoffeeForm(QWidget, Ui_addEditCoffeeForm.Ui_Form, DbBasicFunctions):
    ...
    def save(self):
        ...
        if taste:
            self.create_cursor()
            self.execute_query(
                f"INSERT INTO tastes (taste) VALUES ('{taste}')")
            taste_id = self.execute_query_fetchone(
                f"SELECT id FROM tastes WHERE taste='{taste}'")[0]
            self.close_cursor()
            self.commit_changes()
        ...
        if coffee_id == "AUTO":
            self.execute_query(
                f"INSERT INTO coffee {tuple(query_args)} VALUES {tuple(query_vals)}")
        else:
            print(*zip(query_args, query_vals))
            self.execute_query(f"""UPDATE coffee
            SET {", ".join(map(lambda el: " = ".join(map(lambda e: f"'{e}'", el)), zip(query_args, query_vals)))}
            WHERE id = {coffee_id}""")
        self.close_cursor()
        self.commit_changes()
    ...
    def fix_taste_table(self):
        self.create_cursor()
        self.execute_query_fetchall("""DELETE FROM tastes WHERE
        id NOT IN (
            SELECT taste FROM coffee
    ...
    def prepare_ui(self, coffee_id=None):
        ...
        self.create_cursor()
        types = tuple(el[0] for el in self.execute_query_fetchall(
            f"SELECT type FROM types"))
        ...
        if coffee_id is None:
            ...
            print(coffee_id)
            _, name, type_id, taste_id, price, pack_size = self.execute_query_fetchone(
                f"SELECT * FROM coffee WHERE id={coffee_id}")
            coffee_type = self.execute_query_fetchall(
                f"SELECT type FROM types WHERE id={type_id}")
            if taste_id is not None:
                taste = self.execute_query_fetchone(
                    f"SELECT taste FROM tastes WHERE id={taste_id}")[0]
            else:
                taste = ""
            print(name, type_id, taste_id, price, pack_size, types)
            self.lineEdit_id.setText(str(coffee_id))
